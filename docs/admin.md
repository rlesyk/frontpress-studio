...
- **Theme editor** — three editing surfaces depending on the file extension. **`.fp.json`** (visual templates, see [templates docs]({{ '/templates#visual-templates-fpjson' | relative_url }})) open in the **BlockComposer**: palette of built-in blocks on the left, live iframe canvas in the middle, structured inspector on the right, optional bottom code panel for the selected block's HTML/CSS/JS. **`.html`** files open in a GrapesJS visual editor (lazy-loaded — a 15 MB dependency, only paid for when an `.html` file is opened). Everything else (`.twig`, `.php`, `.css`, etc.) opens in CodeMirror 6 with a live preview iframe alongside. The header offers **+ New visual template** (creates `templates/<name>.fp.json` from a blank tree) and, when a `.twig` / `.php` / `.html` file is open, **Convert to visual** (one-way parse to a `.fp.json` sibling; original kept).
